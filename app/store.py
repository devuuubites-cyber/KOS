...
def process_book(document_id):
    ...
    try:
        ...
        meta['processing_status']='COMPLETE'; meta['error']=None; _write_json(metadata_path,meta)
        result={'status':'COMPLETE','completed_at':utc_now(),'blocks':len(blocks),'chapters':len(chapters),'sections':len(sections),'chunks':len(chunks)}; _write_json(dest/'processing'/'result.json',result); return meta
