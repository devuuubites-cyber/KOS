...
def process_book(document_id):
    ...
    try:
        ...
        db.save_book(document_id,meta['title'],meta.get('author'),meta['file_type'],str(original),meta); db.save_structure(document_id,chapters,sections,chunks)
        db.set_job(document_id,'EXTRACTING','COMPLETE'); db.set_job(document_id,'CHUNKING','COMPLETE',f'{len(chunks)} chunks'); db.set_job(document_id,'INDEXING','COMPLETE')
        meta['processing_status']='COMPLETE'; meta['error']=None; _write_json(metadata_path,meta); _write_json(dest/'processing'/'result.json',{'status':'COMPLETE','completed_at':utc_now(),'blocks':len(blocks),'chapters':len(chapters),'sections':len(sections),'chunks':len(chunks)}); return meta
    except Exception as exc:
        meta['processing_status']='FAILED'; meta['error']=f'{type(exc).__name__}: {exc}'; _write_json(metadata_path,meta); db.set_job(document_id,'EXTRACTING','FAILED',meta['error']); raise
    finally: db.close()

def list_books():
    return [json.loads((d/'metadata.json').read_text(encoding='utf-8')) for d in sorted(BOOKS_DIR.iterdir() if BOOKS_DIR.exists() else [],key=lambda x:x.stat().st_mtime,reverse=True) if d.is_dir() and (d/'metadata.json').exists()]
