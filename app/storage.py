...
CREATE TABLE IF NOT EXISTS relationships(id TEXT PRIMARY KEY,from_object_id TEXT NOT NULL REFERENCES knowledge_objects(id) ON DELETE CASCADE,to_object_id TEXT NOT NULL REFERENCES knowledge_objects(id) ON DELETE CASCADE,type TEXT NOT NULL);
...
class Store:
 ...
 def save_structure(self,book_id,chapters:list[Section],sections:list[Section],chunks:list[Chunk]):
  ...
  for c in chunks:self.conn.execute('INSERT OR REPLACE INTO chunks VALUES(?,?,?,?,?,?,?,?)',(c.chunk_id,book_id,c.chapter,c.section,c.page_start,c.page_end,c.sequence,c.text)); self.conn.execute('DELETE FROM chunks_fts WHERE chunk_id=?',(c.chunk_id,)); self.conn.execute('INSERT INTO chunks_fts VALUES(?,?,?)',(c.chunk_id,book_id,c.text))
  self.conn.commit()
 def save_knowledge(self,book_id,objects:list[dict]):
  for o in objects:
   self.conn.execute('INSERT OR REPLACE INTO knowledge_objects VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',(o['id'],book_id,o['type'],o['title'],o['short_statement'],o['detailed_explanation'],o['importance'],o['confidence'],json.dumps(o.get('domains',[])),json.dumps(o.get('tags',[])),json.dumps(o.get('applications',[])),json.dumps(o.get('source')),o.get('knowledge_status','SOURCE_DERIVED'),o.get('claim_status','AUTHOR_CLAIM')))
   self.conn.execute('DELETE FROM knowledge_fts WHERE object_id=?',(o['id'],)); self.conn.execute('INSERT INTO knowledge_fts VALUES(?,?,?,?,?,?)',(o['id'],book_id,o['title'],o['short_statement'],o['detailed_explanation'],' '.join(o.get('tags',[]))))
  self.conn.commit()
 ...
 def search_knowledge(self,query,limit=20):
  rows=self.conn.execute('SELECT k.* FROM knowledge_fts f JOIN knowledge_objects k ON k.id=f.object_id WHERE knowledge_fts MATCH ? ORDER BY bm25(knowledge_fts) LIMIT ?',(query,limit)).fetchall(); return [dict(r) for r in rows]
